...
postsRouter.get("/", async (req, res, next) => {
  try {
    const aggregation = await Post.aggregate([
      ...
      {
        $replaceWith: {
          $setField: { field: "user", input: "$$ROOT", value: "$_id.user" },
        },
      },
      {
        $replaceWith: {
    ...
    const getPosts = await Post.populate<{ user: { username: string } }>(
      aggregation,
      {
        path: "user",
        select: "username",
...
postsRouter.delete("/:id", auth, async (req: Req, res, next) => {
  try {
    ...
    if (!post.user.equals(user._id)) {
      res
        .status(400)
        .send({ success: false, message: "This post does not belong to you" });
      return;
    }
